 ...
 */
export const securityGroup = async ({ name, description, vpcId, ingress, egress, tags }) => {
	if (!name)
	...
	const securityGroupId = await resolve(sg.id)
	...
	return {
		securityGroup: leanifySecurityGroup(sg),
		securityGroupRules
	}
}
